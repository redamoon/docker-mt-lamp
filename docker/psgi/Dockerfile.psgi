...
# PSGI関連モジュール
# ===========================
RUN dnf -y install --skip-broken "perl(CGI::Parse::PSGI)" "perl(Plack)" "perl(CGI)"
RUN dnf -y install --skip-broken "perl(SOAP::Transport::HTTP)" "perl(XMLRPC::Transport::HTTP)"
RUN cpan -f Plack CGI::PSGI CGI::Parse::PSGI XMLRPC::Transport::HTTP::Plack Starman
...
# PSGI起動スクリプト
# ===========================
COPY psgi/start-psgi.sh /var/www/local/cgi-bin/mt/
RUN chmod +x /var/www/local/cgi-bin/mt/start-psgi.sh
