...
このリポジトリで Movable Type のローカル環境を構築・運用するための手順です。人間のオンボーディングにも、AI エージェントの作業手順にも使います。
...
2. `.env` の環境変数を設定する（既存の `.env` には `COMPOSE_PROFILES=cgi` を追加する）
3. `mt-settings/mt-config.cgi` に DB 情報を入力する（PSGI モードは版ごとの設定ファイルも同様）
4. Movable Type 本体のディレクトリ名を `MT-7.0` にして zip にする
5. `docker/mt-data` 配下に Movable Type を zip で配置する
...
必要なイメージをダウンロードし、設置した Movable Type を配置・展開します。
...
Docker を起動すると Data API 用の Swagger UI が表示されます。
localhost:10000 の MT に入っているデータを確認する場合は、`mt-config.cgi` に `DataAPICORSAllowOrigin` を設定します。
...
Movable Type の Data API ドキュメントでも利用している Redocly をローカルで参照できます。

`data_api.sh` で最新 API の JSON を取得して、起動時に `openapi.json` を参照して閲覧できます。

### Data API OpenAPI

Data API のリポジトリから OpenAPI の JSON を取得するシェルスクリプトです。
...
ローカルで構築した CMS のデータを共有する手順です。

1. 出力された画像などを zip にまとめる
2. git 経由で zip データを共有する（共有先は任意）
3. バックアップした sql を共有
4. zip を解凍して該当のドキュメントルートへ入れる
5. sql をリストアしてデータベースを上書きする
...
Docker で作成した Storage が残っていた場合は削除するコマンドです。
